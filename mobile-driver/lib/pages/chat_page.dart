...
/// ChatPage — Real-time chat between driver and user for a specific order.
/// Styled like a modern messaging app (Gojek-style), driver perspective.
class ChatPage extends StatefulWidget {
  final int orderId;
  final String userName;

  const ChatPage({super.key, required this.orderId, required this.userName});
...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _initChat() async {
    _driverId = await _sessionManager.getUserId();
    _driverName = await _sessionManager.getName();
  ...
  void _sendMessage() {
    ...
    if (text.isEmpty || _driverId == null) return;

    _chatService.sendMessage(
      orderId: widget.orderId,
      senderType: 'DRIVER',
      senderId: int.parse(_driverId!),
